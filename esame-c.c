        ...
        for (i = 0; i < nfds; i++)
                if (fdinfo[fds[i].fd].tcb->st >= ESTABLISHED)
                        if (fds[i].events & POLLIN)
                                if (fdinfo[fds[i].fd].tcb->cumulativeack - fdinfo[fds[i].fd].tcb->rx_win_start)
                                {
...
void app_handler(int signum)
{
        ...
        if (mypoll(myfd, myfdn, 0) > 0)
                for (i = 0; i < myfdn; i++)
                        ...
                        {
                                printf("POLLIN checked!\n");
                                for (; 0 < (t = myread(myfd[i].fd, httpresp + w, 500000 - w)); w += t)
                                        ;
                                printf(">>>>> w= %d\n", w);
                                myfd[0].revents = 0;
                        }
        if ((t == -1) && (myerrno != EAGAIN))
                perror("Reading mysockets");
        if (t == 0)
        {
                printf(">>>> is finished =1\n");
                is_finished = 1;
        }
}
